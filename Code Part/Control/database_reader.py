"""Reader function
Read all the data in a sheet and return it to an object list"""
...
def take_customer_info():
    ...
    for row in sheet.iter_rows(min_row=2):
        ...
        new_customer.set_customer(row[0].value, row[1].value, row[2].value, row[3].value, row[4].value)
...
def take_vehicle_info():
    ...
    for row in sheet.iter_rows(min_row=2):
        ...
        new_vehicle.set_vehicle(row[0].value, row[1].value, row[2].value, row[3].value, row[4].value)
