...
def write_data(system):
    ...
    # rewirte data for "Invoice" sheet:
    sheet4 = data["Invoice"]
    sheet4.delete_rows(2, sheet4.max_row-1)
    for inv in system.get_list("invoice"):
        sheet4.append((inv.get_customer_id(), inv.get_id(), inv.get_driver_id(), inv.get_date().date(), inv.get_payment_mode(), inv.get_distance(), inv.get_price_per_km(), inv.get_total()))
