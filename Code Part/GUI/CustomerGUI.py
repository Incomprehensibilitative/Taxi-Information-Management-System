...
def customer(window, system):
    def load_data():
        ...
        for customer in system.get_list("customer"):
            treeview.insert('', tk.END, values=(
                ...
                customer.get_pick_up_spot()))

    def delete_customer_data():
        id = id_entry.get()
        customer_name, customer_phone_num, customer_chosen_vehicle, customer_pick_up_spot = get_.customer_data(system,
                                                                                                               id)
    ...
    # Select button
    select_button = ttk.Button(user_info_frame, text="Select data", command=select_customer_data,
                               style="Accent.TButton")
    select_button.grid(row=3, column=0, sticky="news", padx=20, pady=10)

    # Update button
    update_button = ttk.Button(user_info_frame, text="Update data", command=update_customer_data,
                               style="Accent.TButton")
    ...
    # Delete button
    delete_button = ttk.Button(delete_user_info_frame, text="Delete data", command=delete_customer_data,
                               style="Accent.TButton")
